    ...
    for item in itemset:
        ...
        if (freq1 > 0) or (freq2 > 0):
            ...
            if freq in k_best:
                ...
                dico = prefixSpan_Recursive2(dataset1, dataset2, itemset, dico, branch_copy, P, N, min_diff, lastseq,
                                             lastfreq1, lastfreq2)
            elif freq > k_best[0]:
...
def prefixSpan2(dataset1, dataset2, itemset, k):
    ...
    dico = prefixSpan_Recursive2(dataset1, dataset2, itemset, dico, branch, P, N, diff_k_best, lastseq, lastfreq1,
                                 lastfreq2)
    dico.pop('')
    k_best = dico['k_first']
    dico.pop('k_first')
            ...
            for m in seq[k]:
                for n in range(a[-1], len(seq[i])):
                    if m == seq[i][n]:
                        a.append(n + 1)
                        break
            # print("seq[K] = ", seq[k],"seq[i] = ", seq[i], "a : ", a)
...
def q3_print(positive, negative, k):
    """
    ...
    dico, seq = prefixSpan2(seq_pos, seq_neg, itemset, k)
