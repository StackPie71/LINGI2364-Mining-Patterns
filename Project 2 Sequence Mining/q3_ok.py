...
def wracc_closed_recursive(dataset1, dataset2, itemset, dico, branch, P, N, min_diff):
    # Recursion
    for item in itemset:
        ...
        if (freq1 > 0) or (freq2 > 0):
            ...
            if freq in k_best:
                ...
                dico = wracc_closed_recursive(dataset1, dataset2, itemset, dico, branch_copy, P, N, min_diff)
            elif freq > k_best[0]:
...
def wracc_closed(dataset1, dataset2, itemset, k):
    # Initialisation
    solution = {}
    x1_init = []
    x2_init = []
    ...
    dico = wracc_closed_recursive(dataset1, dataset2, itemset, dico, branch, P, N, diff_k_best)
    dico.pop('')
    k_best = dico['k_first']
    dico.pop('k_first')
...
def q3_print_nils(positive, negative, k):
    """
    ...
    dico = wracc_closed(seq_pos, seq_neg, itemset, k)
    dico = keep_closed(dico)
    f = open('q3_large.txt', 'w')
