...
def prefixSpan_Recursive2(dataset1, dataset2, itemset, dico, branch, P, N, min_diff,lastseq, lastfreq1, lastfreq2):
    # Recursion
    for item in itemset:
        #print("DICO : ", dico)
        ...
        if (freq1 > 0) or (freq2 > 0) :
            ...
            coef = (P/(P+N)) * (N/(P+N))
            Wracc = abs(coef * ((freq1/P)-(freq2/N)))
            Wracc = round(Wracc, 5)
            freq = Wracc
            #freq = freq1 + freq2
            ...
            if freq in k_best :
                ...
                dico = prefixSpan_Recursive2(dataset1, dataset2, itemset, dico, branch_copy,P, N, min_diff,lastseq, lastfreq1, lastfreq2)
            elif freq > k_best[0] :
                if k_best[0] == -1 or len(min_diff) < len(k_best):
                    # We create our array of minimal born
                    min_diff[freq] = abs(freq1 - freq2)
                else :
                    # If our array is correctly created, we change the minimal born
                    min_diff.pop(k_best[0])
                    min_diff[freq] = abs(freq1 - freq2) #Wracc #freq1 - freq2
                # Update ou k_best
                k_best[0] = freq
...
def prefixSpan2(dataset1, dataset2, itemset, k):
    ...
    dico = prefixSpan_Recursive2(dataset1, dataset2, itemset, dico, branch, P, N, diff_k_best, lastseq, lastfreq1, lastfreq2)
    dico.pop('')
    k_best = dico['k_first']
    dico.pop('k_first')
    seq = []
...
def closed(solution, seq) : 
    seq = sort_seq(seq)
    k = len(seq) -1
    while (k >= 0) :
        for i in range(k) :
            seq_k = solution[seq[k]]
            seq_i = solution[seq[i]]
            a = [0]

            for m in seq[k] :
                for n in range(a[-1], len(seq[i])) :
                    if m == seq[i][n] :
                        a.append(n+1)
                        break
            if (len(a)-1) == len(seq[k]) :
                if (seq_k[0] == seq_i[0]) and (seq_k[1] == seq_i[1]):
                    solution.pop(seq[k])
                    break
        k -= 1
    return solution
...
def q4_print(positive, negative, k):
    """
    ...
    dico, seq = prefixSpan2(seq_neg, seq_pos, itemset, k)
    dico = closed(dico, seq)
    ###
    f = open('q4_large.txt', 'w')
...
start = time.time()
q1_print(negative, positive, 6) #7 
end = time.time()
