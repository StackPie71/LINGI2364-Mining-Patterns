...
##                             Point 3                             ##
#####################################################################  
def do_prediction(i, kbest_patterns, subsets) :
	pos_support_all = subsets[0].tolist()
	neg_support_all = subsets[2].tolist()

	for pat in kbest_patterns :
		if (i in pat[1][1]) or (i in pat[1][3]) :
			if len(pat[1][0]) >= len(pat[1][2]) :
				pred = 1
			else :
				pred = -1
			return pred
...
def train_and_evaluate(minsup, k, database, subsets):
	...
	for i in range(k) :
		# k times
		tmp_k = 1 # Call gSpan with a value of 1 for k
		...
		if len(task.patterns) > 0 :
			...
			for i in range(len(subsets)) :
				tmp_subsets[i] = list(set(tmp_subsets[i]) - set(tmp_pattern[1][i]))  

	# Printing patterns along with their confidences and tot_support:
	for pat in kbest_patterns :
		pos_support = len(pat[1][0])
		neg_support = len(pat[1][2])
		tot_support = pos_support + neg_support
		pos_conf = pos_support / tot_support
		neg_conf = neg_support / tot_support
...
# POINT 3 :  python q3.py data/molecules-small.pos data/molecules-small.neg 5 5 4
