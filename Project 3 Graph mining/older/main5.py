...
class FindingKBestSubgraphs(PatternGraphs):
	...
	def __init__(self, minsup, k, database, subsets):
		...
		"""
		super().__init__(database)
		self.patterns = {}  # The patterns found in the end (as dfs codes represented by strings) with their cover (
		# as a list of graph ids).
		self.minsup = minsup
		self.k = k
		self.gid_subsets = subsets
	...
	# 	# pos_conf = pos_conf_first * 1000 + tot_support # To manage with the total support
	#
	# 	print("----------------------")
	# 	print("gid_subet:", gid_subsets)
	# 	print("pos sup:", pos_support, "neg sup:", neg_support)
	# 	print("tot sup:", tot_support)
	# 	print("pos_conf_first:", pos_conf_first)
	# 	print("pos conf", pos_conf)
	# 	print("---------------------")
	#
	# 	min_val = min(self.dico.values())
	# 	if len(self.patterns) < self.k :
	# 		# If we don't have enough 'k best patterns'
	# 		if pos_conf in self.dico.values() :
	# 			# If pos_conf is already in dico
	# 			key_list = [k  for (k, val) in self.dico.items() if val == pos_conf]
	# 			self.patterns[key_list[0]].append((dfs_code, gid_subsets))
	# 		else :
	# 			# If pos_conf is no in dico, we add it
	# 			key_list = [k  for (k, val) in self.dico.items() if val == min_val]
	# 			self.dico[key_list[0]] = pos_conf
	# 			self.patterns[key_list[0]] = [(dfs_code, gid_subsets)]
	#
	# 	if pos_conf >= min_val :
	# 		if pos_conf in self.dico.values() :
	# 			# If confidence already in k best ones, wwe add patterns
	# 			key_list = [k  for (k, val) in self.dico.items() if val == pos_conf]
	# 			self.patterns[key_list[0]].append((dfs_code, gid_subsets))
	# 		else :
	# 			# If confidence not in k best, we remove the min and we add the nwe confidence
	# 			key_list = [k  for (k, val) in self.dico.items() if val == min_val]
	# 			self.patterns.pop(key_list[0])
	# 			self.dico[key_list[0]] = pos_conf
	# 			self.patterns[key_list[0]] = [(dfs_code, gid_subsets)]
	...
	def store(self, dfs_code, gid_subsets):
		...
		# min_val = min(val[0] for val in self.dico.values())
		all_conf = []  # Garde en mémoire toutes les conf. différentes
		min_val = pos_support
		# Boucle pour avoir la min_val et stocker les conf. différentes présentent dans le dico
		for val in self.dico.values():
			if val[0] < min_val:
				min_val = val[0]
			if val[0] not in all_conf:
				all_conf.append(val[0])

		if pos_conf >= min_val:
			# On regarde si on a un pattern qui match avec un autre d'un point de vue supp et conf
			for val in self.dico.values():
				if tot_support == val[1] and pos_conf == val[0]:
					key_list = [k for (k, val) in self.dico.items() if val[0] == pos_conf]
					self.patterns[key_list[0]].append((dfs_code, gid_subsets))

			# If we don't have k best patterns yet, we add this one
			if len(self.patterns) < self.k:
				key_list = [k for (k, val) in self.dico.items() if val[0] == min_val]
				self.dico[key_list[0]] = [pos_conf, tot_support]
				self.patterns[key_list[0]] = [(dfs_code, gid_subsets)]

		# Si jamais il y a plus de k patterns différents, on élimine les pattern avec la conf_min
		if len(self.patterns) > self.k:
			key_list = [k for (k, val) in self.dico.items() if val[0] == min_val]
			self.dico.pop(key_list[0])
			self.patterns.pop(key_list[0])
