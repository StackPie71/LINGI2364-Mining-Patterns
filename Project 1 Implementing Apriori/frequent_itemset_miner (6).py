    ...
    for i in range(len(projectedV_dataset[1:])):
        ...
        for j in projectedV_dataset[1:][i]:
            if j in projectedV_dataset[0]:
                tmp3.append(j)
            if tmp3:  # If we don't find any intersection, we will be < min Frenquency and not use anymore this item (for the
        # current branche)
